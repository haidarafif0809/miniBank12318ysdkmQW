...
$query0 = $db->query("SELECT SUM(total) AS total_penjualan FROM penjualan WHERE cara_bayar = '$data[kode_daftar_akun]' AND status_jual_awal = 'Tunai'");
            ...
            $query2 = $db->query("SELECT SUM(jumlah) AS jumlah_kas_masuk FROM penyetoran WHERE ke_akun = '$data[kode_daftar_akun]'");
...
//total kas 1
            ...
            $query5 = $db->query("SELECT SUM(jumlah) AS jumlah_kas_keluar FROM penarikan WHERE dari_akun = '$data[kode_daftar_akun]'");
